...
Item {
    ...
    // Current workspace label, centred in the pill: a custom name if one is set
    // for this index, otherwise the niri name, otherwise the number. The
    // symmetric width inset keeps the centring honest while leaving the rail its
    // own space on the left, so a long label elides instead of overlapping it.
    Text {
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.verticalCenter: parent.verticalCenter
        width: parent.width - 48

        text: WorkspaceNames.labelFor(Niri.idx) || Niri.name
